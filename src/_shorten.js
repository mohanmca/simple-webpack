...
SummaryTool.summarizeFromUrl("https://news.ycombinator.com/item?id=14244534", function(err, summary) {
...
var content = `Investment banking Back office - A developer perspective.
...
Order becomes trade when it is executed in the market. Trade that is enriched with details for settlement reaches back office applications.
Stock Record creates or updates traded position.
Clearing and settlement function prepares settlement instruction for the trade and sends to exchange/CCP. Some cases, It directly pays money to counterparty by instructing the nostro agent. Mostly, settlement instructions will be in SWIFT
Exchange/CCP receives settlement instructions from parties involved.It matches them and informs clearing and settlement function.
Exchange/CCP debits money from bank account and credits stocks to the CSD or vice versa. It publishes settlement completion to involved parties.
Settlement and clearing function updates the status of settlement in Stock Record to make it reflect what is in CSD.
If Bank plays custodian role, it starts subscribing to the alerts of corporate action and takes necessary steps to service the asset.
`
